...
use axum::body::Bytes;
use rbx_dom_weak::{
    types::{Color3, Ref, UDim, UDim2, Variant, Vector2, Vector3},
...
fn number(value: &Value) -> Result<f64, CompatError> {
    value
        .as_f64()
        .ok_or_else(|| CompatError::Input("expected a number".into()))
}

fn numbers(value: &Value, length: usize) -> Result<Vec<f64>, CompatError> {
    let values = value
        .as_array()
        .ok_or_else(|| CompatError::Input("expected an array".into()))?;
    if values.len() != length {
        return Err(CompatError::Input(format!("expected {length} numbers")));
    }
...
fn to_variant(property: &PropertyValue, refs: &HashMap<String, Ref>) -> Result<Variant, CompatError> {
    ...
    Ok(match kind.as_str() {
        "string" | "contentid" | "content" => Variant::String(
            property
                .value
                .as_str()
                .ok_or_else(|| CompatError::Input("expected a string".into()))?
                .to_owned(),
        ),
        "bool" | "boolean" => Variant::Bool(
            property
                .value
                .as_bool()
                .ok_or_else(|| CompatError::Input("expected a boolean".into()))?,
        ),
        ...
        "vector2" => {
            let v = numbers(&property.value, 2)?;
            Variant::Vector2(Vector2::new(v[0] as f32, v[1] as f32))
        }
        "vector3" => {
            let v = numbers(&property.value, 3)?;
            Variant::Vector3(Vector3::new(v[0] as f32, v[1] as f32, v[2] as f32))
        }
        "color3" => {
            let v = numbers(&property.value, 3)?;
            Variant::Color3(Color3::new(v[0] as f32, v[1] as f32, v[2] as f32))
        }
        "udim" => {
            let v = numbers(&property.value, 2)?;
            Variant::UDim(UDim::new(v[0] as f32, v[1] as i32))
        }
        "udim2" => {
            let v = numbers(&property.value, 4)?;
        ...
        "ref" | "instance" | "instanceref" => {
            let id = property
                .value
                .as_str()
                .ok_or_else(|| CompatError::Input("expected a referent string".into()))?;
            Variant::Ref(
                *refs
                    .get(id)
                    .ok_or_else(|| CompatError::Input(format!("unknown referenced instance `{id}`")))?,
            )
        }
...
fn validate_request(request: &EncodeRequest) -> Result<(), CompatError> {
    if request.instances.is_empty() {
        return Err(CompatError::Input("instances cannot be empty".into()));
    }

    let mut seen = HashMap::new();
    for instance in &request.instances {
        if instance.referent.is_empty() || instance.class_name.is_empty() {
            return Err(CompatError::Input("referent and class_name are required".into()));
        }
        if seen.insert(instance.referent.clone(), ()).is_some() {
            return Err(CompatError::Input(format!(
                "duplicate referent `{}`",
                instance.referent
            )));
        }
    ...
    for instance in &request.instances {
        if let Some(parent) = &instance.parent {
            if !seen.contains_key(parent) {
                return Err(CompatError::Input(format!("missing parent `{parent}`")));
            }
        }
    }
    Ok(())
...
pub fn encode(request: &EncodeRequest) -> Result<Vec<u8>, CompatError> {
    ...
    let refs = request
        .instances
        .iter()
        .map(|i| Ok((i.referent.clone(), parse_ref(&i.referent)?)))
        .collect::<Result<HashMap<_, _>, CompatError>>()?;
    ...
    while !remaining.is_empty() {
        ...
        for instance in remaining {
            ...
            for (name, property) in &instance.properties {
                builder = builder.with_property(name.clone(), to_variant(property, &refs)?);
            }
            let actual = dom.insert(parent, builder);
            inserted.insert(instance.referent.clone(), actual);
        }
        if next.len() == before {
            return Err(CompatError::Input("parent graph contains a cycle".into()));
        }
        remaining = next;
    }

    let mut output = Vec::new();
    rbx_binary::to_writer(&mut output, &dom, &[data_model])
        .map_err(|e| CompatError::Binary(e.to_string()))?;
    Ok(output)
}
...
fn variant_json(value: &Variant) -> (String, Value) {
    match value {
        ...
        Variant::Vector2(v) => ("Vector2".into(), json!([v.x, v.y])),
        Variant::Vector3(v) => ("Vector3".into(), json!([v.x, v.y, v.z])),
        Variant::Color3(v) => ("Color3".into(), json!([v.r, v.g, v.b])),
...
pub fn decode(bytes: &[u8]) -> Result<Value, CompatError> {
    ...
    for instance in dom.descendants_of(root).skip(1) {
        let parent = if instance.parent() == root {
            None
        } else {
            dom.get_by_ref(instance.parent())
                .map(|p| p.referent().to_string())
        };
...
pub fn validate(bytes: &[u8]) -> Result<(), CompatError> {
    rbx_binary::from_reader(Cursor::new(bytes))
        .map(|_| ())
        .map_err(|e| CompatError::Binary(e.to_string()))
}

pub fn bytes_body(bytes: Bytes) -> Result<Vec<u8>, CompatError> {
    if bytes.is_empty() {
        return Err(CompatError::Input("request body is empty".into()));
    }
    Ok(bytes.to_vec())
}
